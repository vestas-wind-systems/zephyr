...
static struct device *canopen_get_can_device(s32_t index)
{
	struct device *device = NULL;

#if CONFIG_CAN_0
	if (index == 0U) {
		device = device_get_binding(DT_CAN_0_NAME);
	}
#endif
#if CONFIG_CAN_1
	if (index == 1U) {
		device = device_get_binding(DT_CAN_1_NAME);
	}
#endif

	if (device == NULL) {
		LOG_ERR("CAN interface %d not found", index);
	}
				   ...
				   u16_t CANbitRate)
{
	...
	device = canopen_get_can_device(CANbaseAddress);

	if (!CANmodule || !rxArray || !txArray || !device) {
		LOG_ERR("failed to initialize CAN module %d", CANbaseAddress);
	...
	if (rxSize > CONFIG_CAN_MAX_FILTER) {
		...
		LOG_INF("excessive number of concurrent CAN RX filters enabled"
			" (needs %d, %d available)", rxSize,
			CONFIG_CAN_MAX_FILTER);
	}
...
void CO_CANverifyErrors(CO_CANmodule_t *CANmodule)
{
	...
	can_get_state(CANmodule->dev, &err_cnt);
