/*
 * Copyright (c) 2019-2025 Vestas Wind Systems A/S
...
#define CAN_FLEXCAN_IFLAGS4 0x0070
...
#define CAN_FLEXCAN_MB_ID_STD_EXT GENMASK(28, 18)
#define CAN_FLEXCAN_MB_ID_EXT     GENMASK(17, 0)
...
#define CAN_FLEXCAN_RX_MASK_STD GENMASK(28, 18)
#define CAN_FLEXCAN_RX_MASK_EXT GENMASK(28, 0)
...
static void can_flexcan_write_rximr(const struct device *dev, uint8_t mbidx, uint32_t rximr)
{
	mem_addr_t rximr_base;

	/* TODO: assert on mbidx */

	rximr_base = DEVICE_MMIO_NAMED_GET(dev, reg_base) + CAN_FLEXCAN_RXIMR_BASE + (mbidx * 4U);
...
static void can_flexcan_write_mb(const struct device *dev, uint8_t mbidx,
				 const struct can_flexcan_mb_header *header, const uint8_t *data,
				 size_t len)
{
	mem_addr_t mb_base = can_flexcan_get_mb_base(dev, mbidx);

	if (header != NULL) {
		sys_write32(header->cs, mb_base + CAN_FLEXCAN_MB_CS);
		sys_write32(header->id, mb_base + CAN_FLEXCAN_MB_ID);
	}
...
static void can_flexcan_read_mb(const struct device *dev, uint8_t mbidx,
				struct can_flexcan_mb_header *header, uint8_t *data, size_t len)
{
	mem_addr_t mb_base = can_flexcan_get_mb_base(dev, mbidx);

	if (header != NULL) {
		header->cs = sys_read32(mb_base + CAN_FLEXCAN_MB_CS);
		header->id = sys_read32(mb_base + CAN_FLEXCAN_MB_ID);
	}
...
static int can_flexcan_set_timing(const struct device *dev, const struct can_timing *timing)
{
	...
	cbt = CAN_FLEXCAN_CBT_BTF | FIELD_PREP(CAN_FLEXCAN_CBT_EPRESDIV, timing->prescaler - 1U) |
				       ...
				       const struct can_timing *timing_data)
{
	...
	/* TODO: update TDCOFF and friends in FDCTRL with locking */
				     ...
				     void *user_data, const struct can_filter *filter)
{
	...
	can_flexcan_write_rximr(dev, 0U /* TODO: use mbidx */, rximr);
...
static inline void can_flexcan_transfer_error_status(const struct device *dev, uint64_t error)
{
	...
	/* 					/\* FLEXCAN_TransferFDAbortSend(config->base,
	 * &data->handle, */
	/* 					 *\/ */
	...
	/* 				} else { */
	/* #endif                                  /\* CONFIG_CAN_FLEXCAN_FD *\/ */
	/* 					/\* FLEXCAN_TransferAbortSend(config->base,
	 * &data->handle, *\/ */
	/* 					/\*
	 * ALLOC_IDX_TO_TXMB_IDX(alloc)); *\/ */
	/* #ifdef CONFIG_CAN_FLEXCAN_FD */
...
static int can_flexcan_init(const struct device *dev)
{
	...
	if (err == -EINVAL) {
		LOG_ERR("unable to calculate timing for bitrate %u, sample-point %u",
			config->common.bitrate, config->common.sample_point);
		return -ENODEV;
	}
	...
	if (UTIL_AND(IS_ENABLED(CONFIG_CAN_FLEXCAN_FD), config->flexcan_fd)) {
		...
		/* Message Buffer RAM block 0 */
		fdctrl |= FIELD_PREP(CAN_FLEXCAN_FDCTRL_MBDSR0, CAN_FLEXCAN_FDCTRL_MBDS_64BYTES);

		if (config->number_of_mb > 32U) {
			/* Message Buffer RAM block 1 */
			fdctrl |= FIELD_PREP(CAN_FLEXCAN_FDCTRL_MBDSR1,
					     CAN_FLEXCAN_FDCTRL_MBDS_64BYTES);
		}

		if (config->number_of_mb > 64U) {
			/* Message Buffer RAM block 2 */
			fdctrl |= FIELD_PREP(CAN_FLEXCAN_FDCTRL_MBDSR2,
					     CAN_FLEXCAN_FDCTRL_MBDS_64BYTES);
		}

		if (config->number_of_mb > 96U) {
			/* Message Buffer RAM block 3 */
			fdctrl |= FIELD_PREP(CAN_FLEXCAN_FDCTRL_MBDSR3,
					     CAN_FLEXCAN_FDCTRL_MBDS_64BYTES);
		}
		...
		if (err == -EINVAL) {
			LOG_ERR("unable to calculate timing for bitrate-data %u, "
				"sample-point-data %u",
				config->common.bitrate_data, config->common.sample_point_data);
			return -ENODEV;
		}
	...
	for (uint8_t i = 0; i < config->number_of_mb; i++) {
		/* Initialize MB control and status word */
	...
	/* TODO: just set state = stopped here? */
...
#define CAN_FLEXCAN_INST_CLOCKS_FROM_CLK_SOURCE(inst)                                              \
...
#define CAN_FLEXCAN_INST_CLOCKS_NO_CLK_SOURCE(inst)                                                \
	.clock_dev = DEVICE_DT_GET(DT_INST_CLOCKS_CTLR(inst)),                                     \
	.clock_subsys = (clock_control_subsys_t)DT_INST_CLOCKS_CELL(inst, name), .clk_source = 0U

#define CAN_FLEXCAN_INST_CLOCKS(inst)                                                              \
	COND_CODE_1(DT_INST_NODE_HAS_PROP(inst, clk_source),                                       \
		(CAN_FLEXCAN_INST_CLOCKS_FROM_CLK_SOURCE(inst)),                                   \
		(CAN_FLEXCAN_INST_CLOCKS_NO_CLK_SOURCE(inst)))
