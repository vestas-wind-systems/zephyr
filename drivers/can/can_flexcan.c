...
static void can_flexcan_mb_irq_enable(const struct device *dev, uint8_t mbidx, bool enable)
{
	...
	imask_num = mbidx / NUM_BITS(uint32_t);
	imask_bit = mbidx % NUM_BITS(uint32_t);

	switch (imask_num) {
	case 0:
		reg = CAN_FLEXCAN_IMASK1;
		break;
	case 1:
		reg = CAN_FLEXCAN_IMASK2;
		break;
	case 2:
		reg = CAN_FLEXCAN_IMASK3;
		break;
	case 3:
		reg = CAN_FLEXCAN_IMASK4;
		break;
	default:
		CODE_UNREACHABLE;
		return;
	...
	imask = can_flexcan_read_reg(dev, reg);
	...
	can_flexcan_write_reg(dev, reg, imask);
...
static void can_flexcan_isr(const struct device *dev)
{
	...
	/* TODO: remove debug */
	can_flexcan_mb_irq_enable(dev, 0U, false);
