 ...
 */
#ifndef ZEPHYR_INCLUDE_SYS_BUF_H_
#define ZEPHYR_INCLUDE_SYS_BUF_H_
...
/**
 * @brief System buffer library
 * @defgroup sys_buf System Buffer Library
 * @since 1.0
 * @version 1.0.0
 * @ingroup os_services
 * @{
...
#if CONFIG_SYS_BUF_ALIGNMENT == 0
#define __sys_buf_align __aligned(sizeof(void *))
#else
#define __sys_buf_align __aligned(CONFIG_SYS_BUF_ALIGNMENT)
#endif

/**
 *  @brief Define a sys_buf_simple stack variable.
 *
 *  This is a helper macro which is used to define a sys_buf_simple object
 *  on the stack.
 *
 *  @param _name Name of the sys_buf_simple object.
 *  @param _size Maximum data storage for the buffer.
 */
#define SYS_BUF_SIMPLE_DEFINE(_name, _size)     \
	uint8_t sys_buf_data_##_name[_size];       \
	struct sys_buf_simple _name = {         \
		.data   = sys_buf_data_##_name, \
		.len    = 0,                    \
		.size   = _size,                \
		.__buf  = sys_buf_data_##_name, \
	}

/**
 *
 * @brief Define a static sys_buf_simple variable.
 *
 * This is a helper macro which is used to define a static sys_buf_simple
 * object.
 *
 * @param _name Name of the sys_buf_simple object.
 * @param _size Maximum data storage for the buffer.
 */
#define SYS_BUF_SIMPLE_DEFINE_STATIC(_name, _size)        \
	static __noinit uint8_t sys_buf_data_##_name[_size]; \
	static struct sys_buf_simple _name = {            \
		.data   = sys_buf_data_##_name,           \
		.len    = 0,                              \
		.size   = _size,                          \
		.__buf  = sys_buf_data_##_name,           \
...
/**
 * @brief Simple system buffer representation.
 *
 * This is a simpler variant of the sys_buf object (in fact sys_buf uses
 * sys_buf_simple internally). It doesn't provide any kind of reference
 ...
 * sys_buf isn't needed and causes too much overhead. This could be e.g.
...
struct sys_buf_simple {
	 ...
	 *
	 * To determine the max length, use sys_buf_simple_max_len(), not #size!
	 */
	uint16_t len;

	/** Amount of data that sys_buf_simple#__buf can store. */
	uint16_t size;
...
/**
 *
 * @brief Define a sys_buf_simple stack variable and get a pointer to it.
 *
 * This is a helper macro which is used to define a sys_buf_simple object on
 * the stack and the get a pointer to it as follows:
 *
 * struct sys_buf_simple *my_buf = SYS_BUF_SIMPLE(10);
 *
 ...
 * @param _size Maximum data storage for the buffer.
 *
 * @return Pointer to stack-allocated sys_buf_simple object.
 */
#define SYS_BUF_SIMPLE(_size)                        \
	((struct sys_buf_simple *)(&(struct {        \
		struct sys_buf_simple buf;           \
		uint8_t data[_size];                 \
	}) {                                         \
		.buf.size = _size,                   \
	}))
...
/**
 * @brief Initialize a sys_buf_simple object.
 *
 * This needs to be called after creating a sys_buf_simple object using
 * the SYS_BUF_SIMPLE macro.
 *
 * @param buf Buffer to initialize.
 * @param reserve_head Headroom to reserve.
 */
static inline void sys_buf_simple_init(struct sys_buf_simple *buf,
				       size_t reserve_head)
{
	if (!buf->__buf) {
...
/**
 * @brief Initialize a sys_buf_simple object with data.
 *
 ...
 */
static inline void sys_buf_simple_reset(struct sys_buf_simple *buf)
{
	buf->len  = 0U;
	buf->data = buf->__buf;
}
 ...
 *
 * Same idea as with sys_buf_simple_remove_mem(), but a helper for operating
 * on 8-bit values.
 *
 ...
 *
 * Same idea as with sys_buf_simple_pull(), but a helper for operating
 * on 8-bit values.
 *
 * @param buf A valid pointer on a buffer.
 *
 ...
 */
static inline uint8_t *sys_buf_simple_tail(const struct sys_buf_simple *buf)
{
...
/**
 * @brief Check maximum sys_buf_simple::len value.
 *
 * This value is depending on the number of bytes being reserved as headroom.
 *
 * @param buf A valid pointer on a buffer
 *
 * @return Number of bytes usable behind the sys_buf_simple::data pointer.
 */
 ...
 */
static inline void sys_buf_simple_save(const struct sys_buf_simple *buf,
				       struct sys_buf_simple_state *state)
{
	state->offset = (uint16_t)sys_buf_simple_headroom(buf);
	state->len = buf->len;
}
 ...
 * by sys_buf_simple_save().
 *
 * @param buf Buffer to which the state should be restored.
 * @param state Stored state.
 */
static inline void sys_buf_simple_restore(struct sys_buf_simple *buf,
					  struct sys_buf_simple_state *state)
{
 ...
 * explicitly set or unset by the sys_buf API user. Such sys_buf is
 * exclusively instantiated via sys_buf_alloc_with_data() function.
 * Reference count mechanism however will behave the same way, and ref
 * count going to 0 will free the sys_buf but no the data pointer in it.
 */
#define SYS_BUF_EXTERNAL_DATA  BIT(0)

/**
 * @brief System buffer representation.
 *
 * This struct is used to represent system buffers. Such buffers are
 * normally defined through the SYS_BUF_POOL_*_DEFINE() APIs and allocated
 * using the sys_buf_alloc() API.
 */
struct sys_buf {
	...
	/** Union for convenience access to the sys_buf_simple members, also
	 * preserving the old API.
	 */
	union {
		/* The ABI of this struct must match sys_buf_simple */
		struct {
			/** Pointer to the start of data in the buffer. */
			uint8_t *data;
		...
		struct sys_buf_simple b;
		/** @endcond */
	};

	/** System metadata for this buffer. Cleared on allocation. */
	uint8_t user_data[] __sys_buf_align;
};

/** @cond INTERNAL_HIDDEN */

struct sys_buf_data_cb {
	uint8_t * __must_check (*alloc)(struct sys_buf *buf, size_t *size,
			   k_timeout_t timeout);
	uint8_t * __must_check (*ref)(struct sys_buf *buf, uint8_t *data);
	void   (*unref)(struct sys_buf *buf, uint8_t *data);
};

struct sys_buf_data_alloc {
	const struct sys_buf_data_cb *cb;
	void *alloc_data;
...
/**
 * @brief System buffer pool representation.
 *
 * This struct is used to represent a pool of system buffers.
 */
struct sys_buf_pool {
	...
	const char *name;
#endif /* CONFIG_SYS_BUF_POOL_USAGE */

	/** Optional destroy callback when buffer is freed. */
	void (*const destroy)(struct sys_buf *buf);

	/** Data allocation handlers. */
	const struct sys_buf_data_alloc *alloc;

	/** Start of buffer storage array */
	struct sys_buf * const __bufs;
};

/** @cond INTERNAL_HIDDEN */
#define SYS_BUF_POOL_USAGE_INIT(_pool, _count) \
	IF_ENABLED(CONFIG_SYS_BUF_POOL_USAGE, (.avail_count = ATOMIC_INIT(_count),)) \
	IF_ENABLED(CONFIG_SYS_BUF_POOL_USAGE, (.name = STRINGIFY(_pool),))

#define SYS_BUF_POOL_INITIALIZER(_pool, _alloc, _bufs, _count, _ud_size, _destroy) \
	{                                                                          \
		...
		SYS_BUF_POOL_USAGE_INIT(_pool, _count)                             \
		.destroy = _destroy,                                               \
		.alloc = _alloc,                                                   \
		.__bufs = (struct sys_buf *)_bufs,                                 \
	}

#define _SYS_BUF_ARRAY_DEFINE(_name, _count, _ud_size)					       \
	struct _sys_buf_##_name { uint8_t b[sizeof(struct sys_buf)];			       \
				  uint8_t ud[_ud_size]; } __sys_buf_align;		       \
	BUILD_ASSERT(_ud_size <= UINT8_MAX);						       \
	BUILD_ASSERT(offsetof(struct sys_buf, user_data) ==				       \
		     offsetof(struct _sys_buf_##_name, ud), "Invalid offset");		       \
	BUILD_ASSERT(__alignof__(struct sys_buf) ==					       \
		     __alignof__(struct _sys_buf_##_name), "Invalid alignment");	       \
	BUILD_ASSERT(sizeof(struct _sys_buf_##_name) ==					       \
		     ROUND_UP(sizeof(struct sys_buf) + _ud_size, __alignof__(struct sys_buf)), \
		     "Size cannot be determined");					       \
	static struct _sys_buf_##_name _sys_buf_##_name[_count] __noinit

extern const struct sys_buf_data_alloc sys_buf_heap_alloc;
/** @endcond */
 ...
 *
 * Defines a sys_buf_pool struct and the necessary memory storage (array of
 * structs) for the needed amount of buffers. After this, the buffers can be
 * accessed from the pool through sys_buf_alloc. The pool is defined as a
 ...
 * the timeout passed to sys_buf_alloc will be always treated as K_NO_WAIT
 ...
 * responsible for eventually calling sys_buf_destroy() to complete the
 ...
 */
#define SYS_BUF_POOL_HEAP_DEFINE(_name, _count, _ud_size, _destroy)          \
	_SYS_BUF_ARRAY_DEFINE(_name, _count, _ud_size);                      \
	static STRUCT_SECTION_ITERABLE(sys_buf_pool, _name) =                \
		SYS_BUF_POOL_INITIALIZER(_name, &sys_buf_heap_alloc,         \
					 _sys_buf_##_name, _count, _ud_size, \
					 _destroy)

/** @cond INTERNAL_HIDDEN */

struct sys_buf_pool_fixed {
	uint8_t *data_pool;
};

extern const struct sys_buf_data_cb sys_buf_fixed_cb;
 ...
 * the data allocation, so the timeout passed to sys_buf_alloc will be
 ...
 * @param _destroy   Optional destroy callback when buffer is freed.
 */
#define SYS_BUF_POOL_FIXED_DEFINE(_name, _count, _data_size, _ud_size, _destroy) \
	_SYS_BUF_ARRAY_DEFINE(_name, _count, _ud_size);                        \
	static uint8_t __noinit sys_buf_data_##_name[_count][_data_size] __sys_buf_align; \
	static const struct sys_buf_pool_fixed sys_buf_fixed_##_name = {       \
		.data_pool = (uint8_t *)sys_buf_data_##_name,                  \
	};                                                                     \
	static const struct sys_buf_data_alloc sys_buf_fixed_alloc_##_name = { \
		.cb = &sys_buf_fixed_cb,                                       \
		.alloc_data = (void *)&sys_buf_fixed_##_name,                  \
		.max_alloc_size = _data_size,                                  \
	};                                                                     \
	static STRUCT_SECTION_ITERABLE(sys_buf_pool, _name) =                  \
		SYS_BUF_POOL_INITIALIZER(_name, &sys_buf_fixed_alloc_##_name,  \
					 _sys_buf_##_name, _count, _ud_size,   \
					 _destroy)

/** @cond INTERNAL_HIDDEN */
extern const struct sys_buf_data_cb sys_buf_var_cb;
/** @endcond */
 ...
 * @param _destroy   Optional destroy callback when buffer is freed.
 */
#define SYS_BUF_POOL_VAR_DEFINE(_name, _count, _data_size, _ud_size, _destroy) \
	_SYS_BUF_ARRAY_DEFINE(_name, _count, _ud_size);                        \
	K_HEAP_DEFINE(sys_buf_mem_pool_##_name, _data_size);                   \
	static const struct sys_buf_data_alloc sys_buf_data_alloc_##_name = {  \
		.cb = &sys_buf_var_cb,                                         \
		.alloc_data = &sys_buf_mem_pool_##_name,                       \
		.max_alloc_size = 0,                                           \
	};                                                                     \
	static STRUCT_SECTION_ITERABLE(sys_buf_pool, _name) =                  \
		SYS_BUF_POOL_INITIALIZER(_name, &sys_buf_data_alloc_##_name,   \
					 _sys_buf_##_name, _count, _ud_size,   \
					 _destroy)
 ...
 * @param _destroy  Optional destroy callback when buffer is freed.
 */
#define SYS_BUF_POOL_DEFINE(_name, _count, _size, _ud_size, _destroy)        \
	SYS_BUF_POOL_FIXED_DEFINE(_name, _count, _size, _ud_size, _destroy)
 ...
 *       for sys_buf_alloc() to fail (return NULL) even if it was given
 *       K_FOREVER.
 *
...
struct sys_buf * __must_check sys_buf_alloc_fixed_debug(struct sys_buf_pool *pool,
							k_timeout_t timeout,
							const char *func,
							int line);
#define sys_buf_alloc_fixed(_pool, _timeout) \
	sys_buf_alloc_fixed_debug(_pool, _timeout, __func__, __LINE__)
#else
struct sys_buf * __must_check sys_buf_alloc_fixed(struct sys_buf_pool *pool,
						  k_timeout_t timeout);
#endif

/**
 * @copydetails sys_buf_alloc_fixed
 */
static inline struct sys_buf * __must_check sys_buf_alloc(struct sys_buf_pool *pool,
							  k_timeout_t timeout)
{
	return sys_buf_alloc_fixed(pool, timeout);
}
 ...
 */
#if defined(CONFIG_SYS_BUF_LOG)
struct sys_buf * __must_check sys_buf_alloc_len_debug(struct sys_buf_pool *pool,
						      size_t size,
						      k_timeout_t timeout,
						      const char *func,
						      int line);
#define sys_buf_alloc_len(_pool, _size, _timeout) \
	sys_buf_alloc_len_debug(_pool, _size, _timeout, __func__, __LINE__)
#else
struct sys_buf * __must_check sys_buf_alloc_len(struct sys_buf_pool *pool,
						size_t size,
						k_timeout_t timeout);
#endif
 ...
 */
#if defined(CONFIG_SYS_BUF_LOG)
struct sys_buf * __must_check sys_buf_alloc_with_data_debug(struct sys_buf_pool *pool,
							    void *data, size_t size,
							    k_timeout_t timeout,
							    const char *func, int line);
#define sys_buf_alloc_with_data(_pool, _data_, _size, _timeout)		\
	sys_buf_alloc_with_data_debug(_pool, _data_, _size, _timeout,	\
				      __func__, __LINE__)
#else
struct sys_buf * __must_check sys_buf_alloc_with_data(struct sys_buf_pool *pool,
						      void *data, size_t size,
						      k_timeout_t timeout);
#endif
 ...
 */
#if defined(CONFIG_SYS_BUF_LOG)
__deprecated struct sys_buf * __must_check sys_buf_get_debug(struct k_fifo *fifo,
							     k_timeout_t timeout,
							     const char *func, int line);
#define	sys_buf_get(_fifo, _timeout) \
	sys_buf_get_debug(_fifo, _timeout, __func__, __LINE__)
#else
__deprecated struct sys_buf * __must_check sys_buf_get(struct k_fifo *fifo,
						       k_timeout_t timeout);
#endif
 ...
 * If no custom destroy callback is given to SYS_BUF_POOL_*_DEFINE() then
 * there is no need to use this API.
 *
 * @param buf Buffer to destroy.
 */
static inline void sys_buf_destroy(struct sys_buf *buf)
{
	struct sys_buf_pool *pool = sys_buf_pool_get(buf->pool_id);

	if (buf->__buf) {
		if (!(buf->flags & SYS_BUF_EXTERNAL_DATA)) {
 ...
 * @return New buffer or NULL if the FIFO is empty.
 */
struct sys_buf * __must_check sys_buf_slist_get(sys_slist_t *list);
 ...
 * @param buf Buffer.
 */
__deprecated void sys_buf_put(struct k_fifo *fifo, struct sys_buf *buf);
...
#define	sys_buf_unref(_buf) \
	sys_buf_unref_debug(_buf, __func__, __LINE__)
#else
 ...
 */
struct sys_buf * __must_check sys_buf_ref(struct sys_buf *buf);
 ...
 */
struct sys_buf * __must_check sys_buf_clone(struct sys_buf *buf,
					    k_timeout_t timeout);
 ...
 */
static inline void * __must_check sys_buf_user_data(const struct sys_buf *buf)
{
 ...
 * @param reserve How much headroom to reserve.
 */
static inline void sys_buf_reserve(struct sys_buf *buf, size_t reserve)
{
	sys_buf_simple_reserve(&buf->b, reserve);
}
 ...
 * @return The original tail of the buffer.
 */
static inline void *sys_buf_add(struct sys_buf *buf, size_t len)
{
	return sys_buf_simple_add(&buf->b, len);
}
 ...
 * @return The original tail of the buffer.
 */
static inline void *sys_buf_add_mem(struct sys_buf *buf, const void *mem,
				    size_t len)
{
	return sys_buf_simple_add_mem(&buf->b, mem, len);
}
 ...
 * @return Pointer to the value added
 */
static inline uint8_t *sys_buf_add_u8(struct sys_buf *buf, uint8_t val)
{
	return sys_buf_simple_add_u8(&buf->b, val);
}
 ...
 * @param val 16-bit value to be added.
 */
static inline void sys_buf_add_le16(struct sys_buf *buf, uint16_t val)
{
	sys_buf_simple_add_le16(&buf->b, val);
}
 ...
 * @param val 16-bit value to be added.
 */
static inline void sys_buf_add_be16(struct sys_buf *buf, uint16_t val)
{
	sys_buf_simple_add_be16(&buf->b, val);
}
 ...
 * @param val 24-bit value to be added.
 */
static inline void sys_buf_add_le24(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_add_le24(&buf->b, val);
}
 ...
 * @param val 24-bit value to be added.
 */
static inline void sys_buf_add_be24(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_add_be24(&buf->b, val);
}
 ...
 * @param val 32-bit value to be added.
 */
static inline void sys_buf_add_le32(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_add_le32(&buf->b, val);
}
 ...
 * @param val 32-bit value to be added.
 */
static inline void sys_buf_add_be32(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_add_be32(&buf->b, val);
}
 ...
 * @param val 40-bit value to be added.
 */
static inline void sys_buf_add_le40(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_add_le40(&buf->b, val);
}
 ...
 * @param val 40-bit value to be added.
 */
static inline void sys_buf_add_be40(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_add_be40(&buf->b, val);
}
 ...
 * @param val 48-bit value to be added.
 */
static inline void sys_buf_add_le48(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_add_le48(&buf->b, val);
}
 ...
 * @param val 48-bit value to be added.
 */
static inline void sys_buf_add_be48(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_add_be48(&buf->b, val);
}
 ...
 * @param val 64-bit value to be added.
 */
static inline void sys_buf_add_le64(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_add_le64(&buf->b, val);
}
 ...
 * @param val 64-bit value to be added.
 */
static inline void sys_buf_add_be64(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_add_be64(&buf->b, val);
}
 ...
 * @return New end of the buffer data.
 */
static inline void *sys_buf_remove_mem(struct sys_buf *buf, size_t len)
{
	return sys_buf_simple_remove_mem(&buf->b, len);
}

/**
 * @brief Remove a 8-bit value from the end of the buffer
 *
 * Same idea as with sys_buf_remove_mem(), but a helper for operating on
 * 8-bit values.
 *
 * @param buf A valid pointer on a buffer.
 *
 * @return The 8-bit removed value
 */
static inline uint8_t sys_buf_remove_u8(struct sys_buf *buf)
{
	return sys_buf_simple_remove_u8(&buf->b);
}
 ...
 * @return 16-bit value converted from little endian to host endian.
 */
static inline uint16_t sys_buf_remove_le16(struct sys_buf *buf)
{
	return sys_buf_simple_remove_le16(&buf->b);
}
 ...
 * @return 16-bit value converted from big endian to host endian.
 */
static inline uint16_t sys_buf_remove_be16(struct sys_buf *buf)
{
	return sys_buf_simple_remove_be16(&buf->b);
}
 ...
 * @return 24-bit value converted from big endian to host endian.
 */
static inline uint32_t sys_buf_remove_be24(struct sys_buf *buf)
{
	return sys_buf_simple_remove_be24(&buf->b);
}
 ...
 * @return 24-bit value converted from little endian to host endian.
 */
static inline uint32_t sys_buf_remove_le24(struct sys_buf *buf)
{
	return sys_buf_simple_remove_le24(&buf->b);
}
 ...
 * @return 32-bit value converted from little endian to host endian.
 */
static inline uint32_t sys_buf_remove_le32(struct sys_buf *buf)
{
	return sys_buf_simple_remove_le32(&buf->b);
}
 ...
 * @return 32-bit value converted from big endian to host endian.
 */
static inline uint32_t sys_buf_remove_be32(struct sys_buf *buf)
{
	return sys_buf_simple_remove_be32(&buf->b);
}
 ...
 * @return 40-bit value converted from little endian to host endian.
 */
static inline uint64_t sys_buf_remove_le40(struct sys_buf *buf)
{
	return sys_buf_simple_remove_le40(&buf->b);
}
 ...
 * @return 40-bit value converted from big endian to host endian.
 */
static inline uint64_t sys_buf_remove_be40(struct sys_buf *buf)
{
	return sys_buf_simple_remove_be40(&buf->b);
}
 ...
 * @return 48-bit value converted from little endian to host endian.
 */
static inline uint64_t sys_buf_remove_le48(struct sys_buf *buf)
{
	return sys_buf_simple_remove_le48(&buf->b);
}
 ...
 * @return 48-bit value converted from big endian to host endian.
 */
static inline uint64_t sys_buf_remove_be48(struct sys_buf *buf)
{
	return sys_buf_simple_remove_be48(&buf->b);
}
 ...
 * @return 64-bit value converted from little endian to host endian.
 */
static inline uint64_t sys_buf_remove_le64(struct sys_buf *buf)
{
	return sys_buf_simple_remove_le64(&buf->b);
}
 ...
 * @return 64-bit value converted from big endian to host endian.
 */
static inline uint64_t sys_buf_remove_be64(struct sys_buf *buf)
{
	return sys_buf_simple_remove_be64(&buf->b);
}
 ...
 * @return The new beginning of the buffer data.
 */
static inline void *sys_buf_push(struct sys_buf *buf, size_t len)
{
	return sys_buf_simple_push(&buf->b, len);
}
 ...
 * @return The new beginning of the buffer data.
 */
static inline void *sys_buf_push_mem(struct sys_buf *buf, const void *mem,
				     size_t len)
{
	return sys_buf_simple_push_mem(&buf->b, mem, len);
}
 ...
 * @param val 8-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_u8(struct sys_buf *buf, uint8_t val)
{
	sys_buf_simple_push_u8(&buf->b, val);
}
 ...
 * @param val 16-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_le16(struct sys_buf *buf, uint16_t val)
{
	sys_buf_simple_push_le16(&buf->b, val);
}
 ...
 * @param val 16-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_be16(struct sys_buf *buf, uint16_t val)
{
	sys_buf_simple_push_be16(&buf->b, val);
}
 ...
 * @param val 24-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_le24(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_push_le24(&buf->b, val);
}
 ...
 * @param val 24-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_be24(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_push_be24(&buf->b, val);
}
 ...
 * @param val 32-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_le32(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_push_le32(&buf->b, val);
}
 ...
 * @param val 32-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_be32(struct sys_buf *buf, uint32_t val)
{
	sys_buf_simple_push_be32(&buf->b, val);
}
 ...
 * @param val 40-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_le40(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_push_le40(&buf->b, val);
}
 ...
 * @param val 40-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_be40(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_push_be40(&buf->b, val);
}
 ...
 * @param val 48-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_le48(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_push_le48(&buf->b, val);
}
 ...
 * @param val 48-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_be48(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_push_be48(&buf->b, val);
}
 ...
 * @param val 64-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_le64(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_push_le64(&buf->b, val);
}
 ...
 * @param val 64-bit value to be pushed to the buffer.
 */
static inline void sys_buf_push_be64(struct sys_buf *buf, uint64_t val)
{
	sys_buf_simple_push_be64(&buf->b, val);
}
 ...
 * @return New beginning of the buffer data.
 */
static inline void *sys_buf_pull(struct sys_buf *buf, size_t len)
{
	return sys_buf_simple_pull(&buf->b, len);
}
 ...
 */
static inline void *sys_buf_pull_mem(struct sys_buf *buf, size_t len)
{
	return sys_buf_simple_pull_mem(&buf->b, len);
}

/**
 * @brief Remove a 8-bit value from the beginning of the buffer
 *
 * Same idea as with sys_buf_pull(), but a helper for operating on
 * 8-bit values.
 *
 * @param buf A valid pointer on a buffer.
 *
 * @return The 8-bit removed value
 */
static inline uint8_t sys_buf_pull_u8(struct sys_buf *buf)
{
	return sys_buf_simple_pull_u8(&buf->b);
}
 ...
 * @return 16-bit value converted from little endian to host endian.
 */
static inline uint16_t sys_buf_pull_le16(struct sys_buf *buf)
{
	return sys_buf_simple_pull_le16(&buf->b);
}
 ...
 * @return 16-bit value converted from big endian to host endian.
 */
static inline uint16_t sys_buf_pull_be16(struct sys_buf *buf)
{
	return sys_buf_simple_pull_be16(&buf->b);
}
 ...
 * @return 24-bit value converted from little endian to host endian.
 */
static inline uint32_t sys_buf_pull_le24(struct sys_buf *buf)
{
	return sys_buf_simple_pull_le24(&buf->b);
}
 ...
 * @return 24-bit value converted from big endian to host endian.
 */
static inline uint32_t sys_buf_pull_be24(struct sys_buf *buf)
{
	return sys_buf_simple_pull_be24(&buf->b);
}
 ...
 * @return 32-bit value converted from little endian to host endian.
 */
static inline uint32_t sys_buf_pull_le32(struct sys_buf *buf)
{
	return sys_buf_simple_pull_le32(&buf->b);
}
 ...
 * @return 32-bit value converted from big endian to host endian.
 */
static inline uint32_t sys_buf_pull_be32(struct sys_buf *buf)
{
	return sys_buf_simple_pull_be32(&buf->b);
}
 ...
 * @return 40-bit value converted from little endian to host endian.
 */
static inline uint64_t sys_buf_pull_le40(struct sys_buf *buf)
{
	return sys_buf_simple_pull_le40(&buf->b);
}
 ...
 * @return 40-bit value converted from big endian to host endian.
 */
static inline uint64_t sys_buf_pull_be40(struct sys_buf *buf)
{
	return sys_buf_simple_pull_be40(&buf->b);
}
 ...
 * @return 48-bit value converted from little endian to host endian.
 */
static inline uint64_t sys_buf_pull_le48(struct sys_buf *buf)
{
	return sys_buf_simple_pull_le48(&buf->b);
}
 ...
 * @return 48-bit value converted from big endian to host endian.
 */
static inline uint64_t sys_buf_pull_be48(struct sys_buf *buf)
{
	return sys_buf_simple_pull_be48(&buf->b);
}
 ...
 * @return 64-bit value converted from little endian to host endian.
 */
static inline uint64_t sys_buf_pull_le64(struct sys_buf *buf)
{
	return sys_buf_simple_pull_le64(&buf->b);
}
 ...
 * @return 64-bit value converted from big endian to host endian.
 */
static inline uint64_t sys_buf_pull_be64(struct sys_buf *buf)
{
	return sys_buf_simple_pull_be64(&buf->b);
}
 ...
 * @return Number of bytes available at the end of the buffer.
 */
static inline size_t sys_buf_tailroom(const struct sys_buf *buf)
{
	return sys_buf_simple_tailroom(&buf->b);
}
 ...
 * @return Number of bytes available in the beginning of the buffer.
 */
static inline size_t sys_buf_headroom(const struct sys_buf *buf)
{
	return sys_buf_simple_headroom(&buf->b);
}

/**
 * @brief Check maximum sys_buf::len value.
 *
 * This value is depending on the number of bytes being reserved as headroom.
 *
 * @param buf A valid pointer on a buffer
 *
 * @return Number of bytes usable behind the sys_buf::data pointer.
 */
static inline uint16_t sys_buf_max_len(const struct sys_buf *buf)
{
	return sys_buf_simple_max_len(&buf->b);
}
 ...
 * @return Tail pointer for the buffer.
 */
static inline uint8_t *sys_buf_tail(const struct sys_buf *buf)
{
	return sys_buf_simple_tail(&buf->b);
}
 ...
 */
struct sys_buf *sys_buf_frag_last(struct sys_buf *frags);
...
struct sys_buf *sys_buf_frag_del_debug(struct sys_buf *parent,
				       struct sys_buf *frag,
				       const char *func, int line);
#define sys_buf_frag_del(_parent, _frag) \
	sys_buf_frag_del_debug(_parent, _frag, __func__, __LINE__)
#else
struct sys_buf *sys_buf_frag_del(struct sys_buf *parent, struct sys_buf *frag);
#endif

/**
 * @brief Copy bytes from sys_buf chain starting at offset to linear buffer
 *
 * Copy (extract) @a len bytes from @a src sys_buf chain, starting from @a
 * offset in it, to a linear buffer @a dst. Return number of bytes actually
 * copied, which may be less than requested, if sys_buf chain doesn't have
 * enough data, or destination buffer is too small.
 *
 * @param dst Destination buffer
 * @param dst_len Destination buffer length
 * @param src Source sys_buf chain
 * @param offset Starting offset to copy from
 * @param len Number of bytes to copy
 * @return number of bytes actually copied
 */
size_t sys_buf_linearize(void *dst, size_t dst_len,
			 const struct sys_buf *src, size_t offset, size_t len);

/**
 * @typedef sys_buf_allocator_cb
 * @brief System buffer allocator callback.
 *
 * @details The allocator callback is called when sys_buf_append_bytes
 * needs to allocate a new sys_buf.
 *
 * @param timeout Affects the action taken should the sys buf pool be empty.
 *        If K_NO_WAIT, then return immediately. If K_FOREVER, then
 *        wait as long as necessary. Otherwise, wait until the specified
 *        timeout.
 * @param user_data The user data given in sys_buf_append_bytes call.
 * @return pointer to allocated sys_buf or NULL on error.
 */
typedef struct sys_buf * __must_check (*sys_buf_allocator_cb)(k_timeout_t timeout,
							      void *user_data);

/**
 * @brief Append data to a list of sys_buf
 *
 * @details Append data to a sys_buf. If there is not enough space in the
 * sys_buf then more sys_buf will be added, unless there are no free sys_buf
 ...
 * @param timeout Timeout is passed to the sys_buf allocator callback.
 * @param allocate_cb When a new sys_buf is required, use this callback.
...
/**
 * @brief Match data with a sys_buf's content
 *
 * @details Compare data with a content of a sys_buf. Provide information about
...
/**
 * @brief Skip N number of bytes in a sys_buf
 *
 ...
 * are removed from the sys_buf list.
 *
 * @param buf System buffer.
 ...
 */
static inline struct sys_buf *sys_buf_skip(struct sys_buf *buf, size_t len)
{
	while (buf && len--) {
		sys_buf_pull_u8(buf);
		if (!buf->len) {
			buf = sys_buf_frag_del(NULL, buf);
		}
	}
 ...
 */
static inline size_t sys_buf_frags_len(const struct sys_buf *buf)
{
	size_t bytes = 0;
...
#endif /* ZEPHYR_INCLUDE_SYS_BUF_H_ */
